...
🤖 Как это работает
Клиент открывает бота и нажимает кнопку «📱 Записаться» – запускается Mini App.

Выбирает услугу, мастера, дату и время.

Свободные слоты отображаются в реальном времени (запрос к /get_slots).

После отправки данные уходят через Telegram.WebApp.sendData.

Бот проверяет лимиты мастера, доступность слота, сохраняет в PostgreSQL и создаёт событие в Google Calendar.

Клиент получает подтверждение, администратор – уведомление.

За день и за час до записи клиент получает напоминание.

Мастер:

Нажимает иконку 👤 в Mini App, вводит пароль.

Видит список своих будущих записей, может отменить запись.
